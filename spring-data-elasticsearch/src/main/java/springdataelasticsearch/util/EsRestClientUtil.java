package springdataelasticsearch.util;
...
public class EsRestClientUtil {
     ...
     * @return
     */
    public Map<String, Object> getOne(String esIndex, String type, String id) {
        GetRequest getRequest = new GetRequest(esIndex, type, id);
        try {
     ...
     * 修改文档-只需要给要修改的字段
     */
    public void updateOne(String index, String type, String id, Map<String, Object> map) {
     ...
     * @param uuidKey   文档中ID字段对应的key值
     */
    public BulkResponse updateDocumentsAsBatch(String index, String type, List<Map<String, Object>> documents, String uuidKey) {
        BulkResponse response = null;
        if (StringUtils.isBlank(index) || CollectionUtils.isEmpty(documents)) {
            log.warn("Es index is blank or documents is empty.");
            return response;
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                bulkRequest.add(new UpdateRequest(index, type, document.get(uuidKey).toString()).doc(document));
            }
            response = client.bulk(bulkRequest, RequestOptions.DEFAULT);
        } catch (Exception e) {
    ...
    public boolean deleteData(String esIndexName, String type, String esId) {
        DeleteRequest deleteRequest = new org.elasticsearch.action.delete.DeleteRequest(esIndexName, type, esId);
    ...
    public SearchHit[] queryDataBig(String esIndex, SearchSourceBuilder sourceBuilder) {
        ...
        try {
            ...
            if (result.getHits().getTotalHits() > 100000) {
